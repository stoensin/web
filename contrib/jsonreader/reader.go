...
func prep(r io.Reader) (s []byte, err error) {
	...
	for i < len(s) {
		...
		case ':':
			//log.Println("gsdfg", string(s[i:i+len("true")]))
			// 替换True False 到 true false
			for {
				i += 1
				if i >= len(s) {
					break
					...
					//log.Println("TF", s[i], string(s[i]))
					//log.Println("1", string(s[i:i+len("true")]))
					if string(s[i:i+len("true")]) == "True" {
						s[i] = 't'
					}
					if string(s[i:i+len("false")]) == "False" {
						s[i] = 'f'
					}
				} else {
					break
